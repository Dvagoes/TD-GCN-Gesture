...
drop_intv = 3 # drop every frame on this interval
# make drop_intv a command argument
...
for i in range(Samples_sum): # 遍历每一个样本
    ...
    for idx in range(num_subject):
        if idx == int(idx_subject) - 1:
            try:
              os.mkdir("DHG14-28_vary_reg/" + str(idx+1))
            except OSError as error:
              print(error)
            try:
              os.mkdir("DHG14-28_vary_reg/" + str(idx+1)+'/val')
            except OSError as error:
              print(error)
            with open("./DHG14-28_vary_reg/" + str(idx+1)+'/val/' \
            ...
            try:
              ...
              print(error)
            try:
              os.mkdir("DHG14-28_vary_reg/" + str(idx+1)+'/train')
            except OSError as error:
              print(error)
            with open("./DHG14-28_vary_reg/" + str(idx+1) + '/train/' \
                      + file_name + ".json", 'w') as f:
                json.dump(data_json, f)
...
for idx in range(num_subject):
    try:
      ...
      print(error)
    with open("./DHG14-28_vary_reg/" + str(idx + 1) + "/" + str(idx + 1) + "train_samples.json", 'w') as t1:
        json.dump(train_data_dict[idx], t1)

    with open("./DHG14-28_vary_reg/" + str(idx + 1) + "/" + str(idx + 1) + "val_samples.json", 'w') as t2:
